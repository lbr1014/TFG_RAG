...
- **.coveragerc**: este archivo es la configuración de Coverage.py (la herramienta que mide la cobertura de tests en una aplicación Flask). Se ha especificado que al calcular la cobertura no se deben incluir los tests ni el entorno virtual. 
- **app.db**: este archivo se corresponde con una base de datos SQLite. En ella se almacenan los usuarios, los documentos y las consultas.  
...
- **Dockerfile.qdrant**: es una imagen Docker personalizada para el servicio de la base de datos vecotiral (qdrant). Partiendo de la imagen qdrant/qdrant:v1.16.3 añade la instalación de curl dentro del contenedor para asegurar su correcto funcionamiento antes de levantar el resto de contenedores. 
...
- **requirements.txt**: en est earchivo se especidfican las dependencias Python necesarias para instalar y ejecutar la aplicación Flask y su entorno.
...
También se puede ejecutar sin usar el contenedor docker, ejecutando el siguiente comando:
```powershell
python -m flask run
```
