...
Versión: 1.0
Descripción: Script realizado con Playwright para automatizar la extracción de
documentos de Contratación del Sector Público.El script abre la plataforma,
navega al "Perfil Contratante", localiza el buscador dentro de iframes, abre el
selector de órganos y selecciona la opción"Junta de Gobierno de la Diputaciñon
Provincial de Burgos". Después segun la consulta elige en que pestaña entrar.
...
def esperar_frame(
    page: Page, selector: str, timeout: int = 60000
) -> Tuple[Optional[Frame], Locator]:
    """
...
def abrir_seleccionar(
    page: Page, frame: Frame, timeout: int = 15_000
) -> Tuple[Frame, Locator]:
    """
    ...
    frame_arbol, loc_arbol = esperar_frame(
        page, r"#tafelTree_maceoArbol_id_1", timeout=timeout
    )
    print(frame_arbol)
...
def eleccion_organo(frame_arbol: Frame, texto_objetivo: str) -> None:
    """
...
def ir_pestana(page: Page, clave: str, timeout: float = 10_000) -> None:
    """
    ...
    try:
        ...
        raise PlaywrightTimeoutError(f"No se pudo abrir la pestaña: {clave}") from err
...
def extraer_licitaciones(page: Page) -> list[dict]:
    """
    ...
    while True:
        ...
        for i in range(total):
            ...
            datos, documentos = extraer_detalles_licitacion(page)
...
def extraer_detalles_licitacion(page: Page) -> tuple[dict, list[dict]]:
    """
...
def main() -> None:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            page.get_by_role(
                "link", name=re.compile(r"^Perfil Contratante$", re.I)
            ).click()
            ...
            frame_arbol, nodo = abrir_seleccionar(page, frame, timeout=30_000)
            ...
            eleccion_organo(frame_arbol, OBJETIVO)
            print("Junta")
            ...
            ir_pestana(page, destino)
            print("Pestaña abierta")

            resultado = []
            if destino == "Licitaciones":
                resultado = extraer_licitaciones(page)
