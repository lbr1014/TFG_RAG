...
\tightlist
	...
	\item Proporcionar la trazabilidad de la respuesta del modelo, mostrando la información documental con la que se alimenta al LLM. 
	\item Incorporar mecanismos automáticos de evaluación que permitan analizar la calidad de las respuestas generadas por el sistema RAG.
\end{itemize}
...
\tightlist
	...
	\end{itemize}
	\item \req{RF-2} Procesado de los documentos: el sistema debe procesar de manera automática los documentos introducidos para prepararlos para que el RAG pueda usarlos.
	\begin{itemize}
	\tightlist
		...
		\item \req{RF-2.2} Conversión del contenido: el sistema debe transformar el texto extraído a un formato homogéneo, concretamente a Markdown para que sea más fácil su procesamiento.
		\begin{itemize}
		\tightlist
	...
	\end{itemize}
	\item \req{RF-7} Construcción del \eng{prompt} con contexto: el sistema debe construir un \eng{prompt}, para alimentar al LLM, que incluya los \eng{chunks} recuperados, la pregunta del usuario e instrucciones de uso del contexto.
	\item \req{RF-8} Generación de respuestas mediante un LLM: el sistema debe enviar el \eng{prompt} generado a un modelo LLM y mostrar la respuesta de dicho modelo al usuario.
	\begin{itemize}
	\tightlist
		\item \req{RF-8.1} Selección de modelo: el sistema debe permitir utilizar distintos modelos LLM.
	...
	\item \req{RF-14} Evaluación del sistema RAG: el sistema debe permitir evaluar automáticamente la calidad de las respuestas generadas. 
	\begin{itemize}
	\tightlist
...
\tightlist
	...
	\tightlist
		...
		\item \req{RNF-3.2} \eng{Timeouts}: el sistema debe manejar los \eng{timeouts} del LLM devolviendo mensajes controlados al usuario.
	\end{itemize}
	...
	\item \req{RNF-11} Calidad de las respuestas del LLM: el sistema debe llevar a cabo un control de calidad de las respuestas que da el LLM a las consultas del usuario.
	...
	\item \req{RNF-13} Modularidad: el sistema debe permitir sustituir componentes como el LLM, el modelo de \eng{embeddings} o la base de datos vectorial con cambios mínimos.
	\item \req{RNF-14} Portabilidad: la aplicación debe poder desplegarse en distintos entornos utilizando contenedores Docker.
...
\tightlist
	...
	\item \textbf{CU-05} Realizar consulta al RAG (ver tabla~\ref{req-05}).
	...
	\item \textbf{CU-11} Convertir los documentos a Markdown (ver tabla~\ref{req-11}).
	...
	\item \textbf{CU-26} Ejecutar evaluación RAG (ver tabla~\ref{req-26}).
\end{itemize}
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.21\columnwidth} p{0.71\columnwidth} }
		\toprule
		\textbf{\textbf{CU-05}}    & \textbf{Realizar consulta al RAG}\\
		\toprule
		\textbf{Versión}              & 1.0    \\
		\textbf{Autor}                & Lydia Blanco Ruiz \\
		...
		\begin{enumerate}
			...
			\item \eng{Timeout} del LLM $\Rightarrow$ Mostrar un mensaje controlado. 	
		\end{enumerate}\\
		\textbf{Importancia}          & Alta.  \\
		\bottomrule
	\end{tabularx}
	\caption{\textbf{CU-05} Realizar consulta al RAG.}
	\label{req-05}
\end{table}
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.21\columnwidth} p{0.71\columnwidth} }
		\toprule
		\textbf{\textbf{CU-11}}    & \textbf{Convertir los documentos a Markdown}\\
		\toprule
		\textbf{Versión}              & 1.0    \\
		\textbf{Autor}                & Lydia Blanco Ruiz \\
		...
		\begin{enumerate}
			...
			\item Convierte el texto a Markdown homogéneo.
		...
		\end{enumerate}\\
		\textbf{Postcondición}        & Los documentos convertidos a Markdown listos para el \eng{chunking}. \\
		\textbf{Excepciones}          & 
		\begin{enumerate}
			\def\labelenumi{\arabic{enumi}.}
			\tightlist
		...
		\bottomrule
	\end{tabularx}
	\caption{\textbf{CU-11} Convertir los documentos a Markdown.}
	\label{req-11}
\end{table}
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.21\columnwidth} p{0.71\columnwidth} }
		...
		\begin{enumerate}
			...
			\item El sistema envía el \eng{prompt} al LLM.
		...
		\begin{enumerate}
			...
			\item El LLM no responde o se produce un \eng{timeout} $\Rightarrow$ Muestra un mensaje de error controlado.
		\end{enumerate}\\
		\textbf{Importancia}          & Alta.  \\
		\bottomrule
	\end{tabularx}
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.21\columnwidth} p{0.71\columnwidth} }
		...
		\textbf{Descripción}          & Permite visualizar estadísticas y gráficas relacionadas con el uso de la aplicación por parte de los usuarios y comparativas entre los modelos LLM cargados en el sistema. \\
		...
		\begin{enumerate}
			...
			\item El sistema genera gráficas y métricas mediante D3.js.
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.21\columnwidth} p{0.71\columnwidth} }
		\toprule
		\textbf{\textbf{CU-26}}    & \textbf{ Ejecutar evaluación RAG}\\
		\toprule
		\textbf{Versión}              & 1.0    \\
		\textbf{Autor}                & Lydia Blanco Ruiz \\
		\textbf{Requisitos asociados} & \refreq{RF-14}, \refreq{RF-14.1}, \refreq{RNF-11} \\
		\textbf{Descripción}          & Permite al administrador ejecutar evaluaciones automáticas sobre la calidad del RAG. \\
		...
		\begin{enumerate}
			...
			\item El modelo LLM no responde $\Rightarrow$ Muestra un mensaje de \eng{timeout} o error controlado.
		\end{enumerate}\\
		\textbf{Importancia}          & Alta.  \\
		\bottomrule
	\end{tabularx}
	\caption{\textbf{CU-26} Ejecutar evaluación RAG.}
	\label{req-26}
\end{table}