...
\subsection{Protección de datos personales}\label{subsec:proctdatos}
...
\subsection{Propiedad intelectual y derechos de autor}\label{subsec:propiedad}
...
Además, el proyecto utiliza múltiples bibliotecas y herramientas de terceros, como Flasl, Docker, Qdrant, Playwright, Selenium y Ollama. Todas estas herramientas emplean licencias \eng{open source} compatibles con el desarrollo académico y de investigación (ver tabla~\ref{tabla:licencias}). Por ello, el proyecto es legalmente viable siempre que se mantengan las atribuciones correspondientes, se respeten las condiciones de redistribución de cada licencia y no se eliminen los avisos de \eng{copyright} de los terceros.

\begin{table}[p]
	\centering
	\begin{tabularx}{\linewidth}{ p{0.2\columnwidth} p{0.2\columnwidth} p{0.52\columnwidth} }
		\toprule
		\textbf{Herramienta} & \textbf{Licencia} & \textbf{Características} \\
		\toprule
	...
	SonarQube / SonarCloud Scanner & LGPL v3 & Herramienta de análisis estático y calidad del código \\
		\bottomrule
	\end{tabularx}
	\caption{Licencias de las herramientas de la aplicación.}
	\label{tabla:licencias}
\end{table}

\subsection{Legalidad del \eng{Web Scraping}}\label{subsec:webScraping}

El proyecto incorpora técnicas de \eng{Web Scraping} para recopilar licitaciones públicas desde la Plataforma de \contsec{}~\cite{legal:contratosSecPub}. La información extraída es pública y accesible libremente, por lo que su utilización tiene encaje legal siempre que no se vulneren mecanismos de autenticación o acceso restringido, no se produzca una sobrecarga intencionada de los servidores, se respeten las condiciones de uso del protal y se utilice la información con fines académicos y de investigación. 

Según el \file{robots.txt}~\cite{legal:robots}

\subsection{Uso de modelos de Inteligencia Artificial}\label{subsec:IA}
...
\subsection{Seguridad Informática}\label{subsec:seg}

El despliegue del sistema utilizando \file{Docker, Gunicorn, Nginx y HTTPS} permite cumplir buenas prácticas de seguridad. Además, el uso de variables de entorno para secretos, separación entre servicios, control de acceso mediante autenticación y certificados \file{SSL/TLS} reduce el riesgo relacionado con accesos no autorizados o filtraciones de información, cumpliendo asi con el Esquema Nacional de Seguridad (ENS)~\cite{legal:ENS}. 
